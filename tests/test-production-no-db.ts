...
async function testProductionWithoutDatabase() {
  ...
  const config: HumanBehaviorConfig = {
    minDelayBetweenRequests: 10000, // 10 secondes minimum (plus lent)
    maxDelayBetweenRequests: 20000, // 20 secondes maximum (plus lent)
    minDelayBetweenPages: 15000,    // 15 secondes minimum entre pages (plus lent)
    maxDelayBetweenPages: 30000,    // 30 secondes maximum entre pages (plus lent)
    maxPagesPerSession: 1,          // Maximum 1 page par session (très réduit)
    sessionBreakDuration: 60000,    // 1 minute de pause (réduit pour le test)
