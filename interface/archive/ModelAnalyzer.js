...
class ModelAnalyzer {
  constructor(dbManager = null) {
    ...
    // Patterns de détection des modèles iPhone
    this.iphonePatterns = {
      // Modèles principaux
      models: {
  ...
  /**
   * Calcule les prix de référence à partir de la base de données
   */
  async calculateReferencePrices() {
    if (!this.dbManager) {
      console.log('⚠️ DatabaseManager non disponible pour calculer les prix de référence');
      return;
    }

    try {
      console.log('📊 Calcul des prix de référence à partir de la base de données...');
      
      // Récupérer toutes les annonces iPhone
      ...
      if (listings.length === 0) {
        console.log('⚠️ Aucune annonce trouvée pour calculer les prix de référence');
        return;
      }
      ...
      console.log('✅ Prix de référence calculés:', this.referencePrices);
      
    } catch (error) {
      console.error('❌ Erreur lors du calcul des prix de référence:', error);
    }
  }
  ...
  analyzeListing(listing) {
    ...
    if (analysis.model && analysis.storage) {
      console.log(`🔍 Analyse prix pour: "${analysis.model}" "${analysis.storage}" - Prix: ${analysis.price}€`);
      console.log(`📊 Titre original: "${listing.title}"`);
      ...
      console.log(`⚠️ Modèle ou stockage manquant pour: "${listing.title}"`);
      console.log(`📊 Modèle détecté: "${analysis.model}"`);
      console.log(`📊 Stockage détecté: "${analysis.storage}"`);
    }
  ...
  analyzePrice(model, storage, currentPrice) {
    ...
    if (!referencePrice) {
      console.log(`⚠️ Prix de référence manquant pour ${model} ${storage || 'N/A'}`);
      console.log(`📊 Modèles disponibles:`, Object.keys(this.referencePrices));
      if (model && this.referencePrices[model]) {
        console.log(`📊 Stockages disponibles pour ${model}:`, Object.keys(this.referencePrices[model]));
      }
      return {
        isGoodDeal: false,
        dealScore: 0,
  ...
  async analyzeListings(listings) {
    ...
    if (Object.keys(this.referencePrices).length === 0) {
      await this.calculateReferencePrices();
    }
    
    const analyzedListings = listings.map(listing => this.analyzeListing(listing));
